...
def check_if_logged_in():
    ...
    if not session.get('user_id') and (request.endpoint == "member_index" or request.endpoint == "member_article"):
        return make_response({'message':"Error unauthorized"},401)
...
class MemberOnlyIndex(Resource):
    ...
    def get(self):
        member_articles = [article.to_dict() for article in Article.query.filter_by(is_member_only = 1).all()]

        return make_response(member_articles,200)

class MemberOnlyArticle(Resource):
    
    def get(self, id):
        member_article = Article.query.filter_by(is_member_only = 1).filter_by(id =id).first()
